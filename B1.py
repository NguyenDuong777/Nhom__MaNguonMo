...
def choose_csv_and_predict():
    ...
    try:
        ...
        n_neighbors = simpledialog.askinteger("Số lượng hàng xóm", "Nhập số lượng hàng xóm (n_neighbors):", minvalue=1, maxvalue=10)
        if not n_neighbors:
            return  # Exit if no valid input

        # Assume first 5 columns are features and the 6th column is the actual performance
        x = np.array(df.iloc[:, 0:5]).astype(np.float64)  # Features (columns 1-5)
        y = np.array(df.iloc[:, 5:6]).astype(np.float64)  # Labels (column 6 - actual performance)

        # Split data into training and test sets
        X_train, X_test, y_train, y_test = train_test_split(x, y, test_size=0.2, random_state=1)

        # Create the KNN model
        knn = neighbors.KNeighborsRegressor(n_neighbors=n_neighbors, p=2)
...
def input_new_data():
    ...
    def predict_and_compare():
        try:
            # Get input values
            input_values = [float(entry.get()) for entry in entries[:5]]
