...
def clear_entries():
  try:
    n = int(entry_n.get())  # Lấy số ẩn
    for i in range(n):
      for j in range(n):
        coefficient_entries[i][j].delete(0, tk.END)  # Xóa ô nhập liệu hệ số
      result_entries[i].delete(0, tk.END)  # Xóa ô nhập liệu kết quả
  except Exception as e:
